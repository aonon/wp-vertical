...
add_action('add_meta_boxes', 'dazzling_add_custom_box');
/**
 * Add Meta Boxes.
 *
 * Add Meta box in page and post post types.
 */
function dazzling_add_custom_box()
{
    add_meta_box('siderbar-layout', //Unique ID
        __('Select layout for this specific Page only ( Note: This setting only reflects if page Template is set as Default Template and Blog Type Templates.)', 'dazzling'), //Title
        'dazzling_sidebar_layout', //Callback function
        'page' //show metabox in pages
        );
    add_meta_box('siderbar-layout', //Unique ID
        __('Select layout for this specific Post only', 'dazzling'), //Title
        'dazzling_sidebar_layout', //Callback function
 ...
 */
function dazzling_sidebar_layout()
{
    global $site_layout, $post;
    // Use nonce for verification
    wp_nonce_field(basename(__FILE__), 'custom_meta_box_nonce'); ?>
	
    <table id="sidebar-metabox" class="form-table" height="100%">
        <tbody>
            <tr>
...
add_action('save_post', 'dazzling_save_custom_meta');
/**
 * save the custom metabox data
 * @hooked to save_post hook
 */
function dazzling_save_custom_meta($post_id)
{
    global $site_layout, $post;
