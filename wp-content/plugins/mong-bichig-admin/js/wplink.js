...
( function( $ ) {
	...
	River = function( element, search ) {
		...
		this.contentWidth = element.children( '#link-selector-height' );
	...
	$.extend( River.prototype, {
		...
		select: function( li, event ) {
			var liWidth, elWidth, liLeft, elLeft;
			...
			elLeft = this.element.scrollLeft();

			if ( liLeft < 0 ) // Make first visible element
				this.element.scrollLeft( elLeft + liLeft );
			else if ( liLeft + liWidth > elWidth ) // Make last visible element
				this.element.scrollLeft( elLeft + liLeft - elWidth + liWidth );
		...
		maybeLoad: function() {
			...
			if ( ! this.query.ready() || right < this.contentWidth.width() - wpLink.riverRightThreshold )
				return;

			setTimeout(function() {
				var newLeft = el.scrollLeft(),
					newRight = newLeft + el.width();

				if ( ! self.query.ready() || newRight < self.contentWidth.width() - wpLink.riverRightThreshold )
					return;

				self.waiting.addClass( 'is-active' );
				el.scrollLeft( newLeft + self.waiting.outerWidth() );
