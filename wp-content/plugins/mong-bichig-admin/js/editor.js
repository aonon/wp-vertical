...
( function( $ ) {
	function SwitchEditors() {
		...
		function getToolbarWidth( editor ) {
		...
		function switchEditor( id, mode ) {
			...
			var editorWidth, toolbarWidth, iframe,
			...
			if ( 'tmce' === mode || 'tinymce' === mode ) {
				...
				editorWidth = parseInt( textarea.style.width, 10 ) || 0;

				if ( editor ) {
					editor.show();

					// No point resizing the iframe in iOS
					if ( ! tinymce.Env.iOS && editorWidth ) {
						toolbarWidth = getToolbarWidth( editor );
						editorWidth = editorWidth - toolbarWidth + 14;

						// width cannot be under 50 or over 5000
						if ( editorWidth > 50 && editorWidth < 5000 ) {
							editor.theme.resizeTo( editorWidth, null );
						}
					}
				} else {
				...
				if ( editor ) {
					if ( ! tinymce.Env.iOS ) {
						...
						editorWidth = iframe ? parseInt( iframe.style.width, 10 ) : 0;
