...
if ( typeof(jQuery) != 'undefined' ) {
	...
	(function(d, w) {
		...
		scrollToLeft = function(t) {
			...
			distance    = window.pageYOffset || document.documentElement.scrollLeft || document.body.scrollLeft || 0;
			...
			// Animate scrolling to the left of the page by generating steps to
			// the left of the page and shifting to each step at a set interval.
			while ( distance ) {
				distance -= step;
				if ( distance < 0 )
					distance = 0;
		...
		addEvent(w, 'load', function() {
			...
			if ( d.body && aB ) {
				...
				addEvent(aB, 'click', function(e) {
					scrollToLeft( e.target || e.srcElement );
				});
