...
_.extend( media, {
	...
	fit: function( dimensions ) {
		...
		if ( ! _.isUndefined( maxHeight ) && ! _.isUndefined( maxWidth ) ) {
			constraint = ( height / width > maxHeight / maxWidth ) ? 'height' : 'width';
		} else if ( _.isUndefined( maxWidth ) ) {
			constraint = 'height';
		} else if (  _.isUndefined( maxHeight ) && width > maxWidth ) {
			constraint = 'width';
		}

		// If the value of the constrained side is larger than the max,
		// then scale the values. Otherwise return the originals; they fit.
		if ( 'height' === constraint && height > maxHeight ) {
			return {
				height : maxHeight,
				width: Math.round( maxHeight * width / height )
			};
		} else if ( 'width' === constraint && width > maxWidth ) {
			return {
...
var PostImage = Backbone.Model.extend({
	...
	setAspectRatio: function() {
		...
		if ( this.attachment && this.attachment.get( 'sizes' ) ) {
			...
			if ( full ) {
				this.set( 'aspectRatio', full.height / full.width );
				return;
			}
		}

		this.set( 'aspectRatio', this.get( 'customHeight' ) / this.get( 'customWidth' ) );
	}
});
