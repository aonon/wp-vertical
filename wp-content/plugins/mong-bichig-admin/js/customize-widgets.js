...
(function( wp, $ ){
	...
	api.Widgets.WidgetControl = api.Control.extend({
		...
		_setupWideWidget: function() {
			...
			/**
			 * Keep the widget-inside positioned so the left of fixed-positioned
			 * element is at the same left position as the widget-top. When the
			 * widget-top is scrolled out of view, keep the widget-top in view;
			 * likewise, don't allow the widget to drop off the right of the window.
			 * If a widget is too tall to fit in the window, don't let the width
			 * exceed the window width so that the contents of the widget control
			 * will become scrollable (overflow:auto).
			 */
			positionWidget = function() {
				var offsetLeft = self.container.offset().left,
					windowWidth = $( window ).width(),
					formWidth = $widgetForm.outerWidth(),
					left;
				$widgetInside.css( 'maxwidth', windowWidth );
				left = Math.max(
					0, // prevent left from going off screen
					Math.min(
						Math.max( offsetLeft, 0 ), // distance widget in panel is from left of screen
						windowWidth - formWidth // flush up against right of screen
					)
				);
				$widgetInside.css( 'left', left );
			};
		...
		onChangeActive: function ( active, args ) {
			// Note: there is a second 'args' parameter being passed, merged on left of this.defaultActiveArguments
		 ...
		 *
		 * @param {Boolean} expanded
		 * @param {Object} args  merged on left of this.defaultActiveArguments
		 */
