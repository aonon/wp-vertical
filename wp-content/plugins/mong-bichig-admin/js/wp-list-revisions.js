	...
	var init = function() {
		...
		pr.onclick = function() {
			...
			for ( i = 0; i < inputs.length; i++ ) {
				...
				if ( ! inputs[i].checked &&
				( 'top' == side && 1 > checkCount || 'bottom' == side && 1 < checkCount && ( ! inputs[i-1] || ! inputs[i-1].checked ) ) &&
				! ( inputs[i+1] && inputs[i+1].checked && 'bottom' == inputs[i+1].getAttribute('name') ) )
					inputs[i].style.visibility = 'hidden';
				else if ( 'top' == side || 'bottom' == side )
