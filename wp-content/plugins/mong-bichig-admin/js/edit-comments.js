...
commentReply = {
	...
	open : function(comment_id, post_id, action) {
		var editRow, rowData, act, replyButton, editWidth,
			t = this,
		...
		if ( action == 'edit' ) {
			...
			if ( h > 120 ) {
				// Limit the maximum width when editing very long comments to make it more manageable.
				// The textarea is resizable in most browsers, so the user can adjust it if needed.
				editWidth = h > 500 ? 500 : h;
				$('#replycontent', editRow).css('width', editWidth + 'px');
			}
		...
		setTimeout(function() {
			var rtop, rbottom, scrollLeft, vp, scrollRight;

			rtop = $('#replyrow').offset().left;
			rbottom = rtop + $('#replyrow').width();
			scrollLeft = window.pageYOffset || document.documentElement.scrollLeft;
			vp = document.documentElement.clientWidth || window.innerWidth || 0;
			scrollRight = scrollLeft + vp;

			if ( scrollRight - 20 < rbottom )
				window.scroll(0, rbottom - vp + 35);
			else if ( rtop - 20 < scrollLeft )
