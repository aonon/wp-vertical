	...
	function dataToHtml(data) {
		...
		tinymce.each(urlPatterns, function(pattern) {
			...
			if ((match = pattern.regex.exec(data.source1))) {
				...
				data.height = data.height || pattern.w;
				data.width = data.width || pattern.h;
			}
		});
		...
		if (data.embed) {
			...
			if (data.type == "iframe") {
				...
				html += '<iframe src="' + data.source1 + '" height="' + data.height + '" width="' + data.width + '"' + allowFullscreen + '></iframe>';
			} else if (data.source1mime == "application/x-shockwave-flash") {
				html += '<object data="' + data.source1 + '" height="' + data.height + '" width="' + data.width + '" type="application/x-shockwave-flash">';

				if (data.poster) {
					html += '<img src="' + data.poster + '" height="' + data.height + '" width="' + data.width + '" />';
				}
				...
				if (editor.settings.video_template_callback) {
					...
					html = (
						'<video height="' + data.height + '" width="' + data.width + '"' + (data.poster ? ' poster="' + data.poster + '"' : '') + ' controls="controls">\n' +
	...
	editor.on('preInit', function() {
		...
		editor.parser.addNodeFilter('iframe,video,audio,object,embed,script', function(nodes, name) {
			...
			while (i--) {
				...
				// Prefix all attributes except height, width and style since we
				...
				while (ai--) {
					...
					if (attrName !== "height" && attrName !== "width" && attrName !== "style") {
				...
				placeHolder.attr({
					...
					width: node.attr('width') || (name == "audio" ? "30" : "150"),
		...
		editor.serializer.addAttributeFilter('data-mce-object', function(nodes, name) {
			...
			while (i--) {
				...
				// Add height/height to everything but audio
