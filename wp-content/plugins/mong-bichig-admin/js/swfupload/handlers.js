var topWin = window.dialogArguments || opener || parent || left;
...
function uploadProgress(fileObj, bytesDone, bytesTotal) {
	...
	var w = jQuery('#media-items').height() - 2, item = jQuery('#media-item-' + fileObj.id);
	jQuery('.bar', item).height( w * bytesDone / bytesTotal );
...
function prepareMediaItemInit(fileObj) {
	...
	jQuery('a.toggle', item).click(function(){
		jQuery(this).siblings('.slidetoggle').slideToggle(350, function(){
			var w = jQuery(window).width(), t = jQuery(this).offset().left, h = jQuery(this).width(), b;
