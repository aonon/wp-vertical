...
EditAttachments = MediaFrame.extend({
	...
	events: {
		'click .top':  'previousMediaItem',
		'click .bottom': 'nextMediaItem'
	},
	...
	toggleNav: function() {
		this.$('.top').toggleClass( 'disabled', ! this.hasPrevious() );
		this.$('.bottom').toggleClass( 'disabled', ! this.hasNext() );
	},
	...
	/**
	 * Respond to the keyboard events: bottom arrow, top arrow, except when
...
Manage = MediaFrame.extend({
	...
	fixPosition: function() {
		...
		// Offset doesn't appear to take left margin into account, hence +16
		if ( ( $browser.offset().left + 16 ) < this.$window.scrollLeft() + this.$adminBar.width() ) {
			$browser.addClass( 'fixed' );
			$toolbar.css('height', $browser.height() + 'px');
		} else {
