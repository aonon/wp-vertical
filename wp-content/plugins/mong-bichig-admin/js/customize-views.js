(function( $, wp, _ ) {
	...
	api.HeaderTool.CurrentView = wp.Backbone.View.extend({
		...
		getWidth: function() {
			...
			saved = this.model.get('savedWidth');
			width = image.width() || saved;

			// happens at ready
			if (!width) {
				headerImageData = api.get().header_image_data;

				if (headerImageData && headerImageData.height && headerImageData.width) {
					// hardcoded container height
					width = 260 / headerImageData.height * headerImageData.width;
				}
				else {
		...
		setPlaceholder: function(_height) {
			var width = _height || this.getWidth();
			this.model.set('savedWidth', width);
			this.$el.width(width);
		},
	...
	api.HeaderTool.ChoiceView = wp.Backbone.View.extend({
		...
		getWidth: api.HeaderTool.CurrentView.prototype.getWidth,
