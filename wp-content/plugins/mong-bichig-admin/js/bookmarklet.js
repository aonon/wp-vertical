( function( window, document, href, pt_url ) {
	var encURI = window.encodeURIComponent,
		...
		canPost = true,
		windowHeight, windowWidth, selection,
	...
	windowHeight  = window.outerHeight || document.documentElement.clientHeight || 600;
	windowWidth = window.outerWidth || document.documentElement.clientWidth || 700;

	windowHeight = ( windowHeight < 800 || windowHeight > 5000 ) ? 600 : ( windowHeight * 0.7 );
	windowWidth = ( windowWidth < 800 || windowWidth > 3000 ) ? 700 : ( windowWidth * 0.9 );

	if ( ! canPost ) {
		window.open( pt_url, target, 'location,resizable,scrollbars,height=' + windowHeight + ',width=' + windowWidth );
		return;
	}
	...
	for ( var n = 0; n < images.length; n++ ) {
		...
		// If we know the image height and/or width, check them now and drop the "uninteresting" images.
		if ( img.src.indexOf( 'avatar' ) > -1 || img.className.indexOf( 'avatar' ) > -1 ||
			( img.height && img.height < 256 ) || ( img.width && img.width < 128 ) ) {
	...
	window.open( 'about:blank', target, 'location,resizable,scrollbars,height=' + windowHeight + ',width=' + windowWidth );

	document.body.appendChild( form );
	form.submit();
} )( window, document, left.location.href, window.pt_url );
