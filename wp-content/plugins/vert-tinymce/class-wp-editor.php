...
final class _WP_Editors {
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		if ( empty( $set['editor_width'] ) )
			return $set;

		if ( 'content' === $editor_id && empty( $set['tinymce']['wp_autoresize_on'] ) ) {
			// A cookie (set when a user resizes the editor) overrides the width.
			$cookie = (int) get_user_setting( 'ed_size' );

			if ( $cookie )
				$set['editor_width'] = $cookie;
		}

		if ( $set['editor_width'] < 50 )
			$set['editor_width'] = 50;
		elseif ( $set['editor_width'] > 5000 )
			$set['editor_width'] = 5000;
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		if ( ! empty( $set['editor_width'] ) ) {
			$width = ' style="width: ' . (int) $set['editor_width'] . 'px"';
		} else {
			$width = ' rows="' . (int) $set['textarea_rows'] . '"';
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				...
				$buttons .= '<div type="button" id="' . $editor_id_attr . '-tmce" class="wp-switch-editor switch-tmce"' .
					' data-wp-editor-id="' . $editor_id_attr . '">' . __('Visual') . "</div>\n";
				$buttons .= '<div type="button" id="' . $editor_id_attr . '-html" class="wp-switch-editor switch-html"' .
					' data-wp-editor-id="' . $editor_id_attr . '">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</div>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$the_editor = apply_filters( 'the_editor', '<div id="wp-' . $editor_id_attr . '-editor-container" class="wp-editor-container">' .
			$quicktags_toolbar .
			'<textarea' . $editor_class . $width . $tabindex . $autocomplete . ' cols="40" name="' . esc_attr( $set['textarea_name'] ) . '" ' .
	...
	public static function editor_settings($editor_id, $set) {
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				self::$baseurl = plugins_url('', __FILE__) . '/tinymce' ;
				...
				if ( $set['teeny'] ) {
					 ...
					 * @param string $editor_id Unique editor identifier, e.g. 'content'.
					 */
					self::$plugins = $plugins = apply_filters( 'teeny_mce_plugins', array( 'chanai', 'colorpicker', 'lists', 'fullscreen', 'image', 'wordpress', 'wpeditimage' , 'link'), $editor_id );
				} else {
				...
				$dashicons = plugins_url("mong-bichig-admin/css/dashicons$suffix.css?$version" );
			...
			if ( $set['teeny'] ) {
				 ...
				 * @param string $editor_id Unique editor identifier, e.g. 'content'.
				 */
				$mce_buttons = apply_filters( 'teeny_mce_buttons', array('bold', 'italic', 'underline', 'blockquote', 'strikethrough', 'bullist', 'numlist', 'alignleft', 'aligncenter', 'alignright', 'undo', 'redo', 'link', 'unlink', 'fontselect', 'fullscreen'), $editor_id );
				$mce_buttons_2 = $mce_buttons_3 = $mce_buttons_4 = array();
			} else {
				$mce_buttons = array( 'bold', 'italic', 'strikethrough', 'bullist', 'numlist', 'blockquote', 'hr', 'alignleft', 'aligncenter', 'alignright', 'link', 'unlink', 'wp_more', 'fullscreen' );
				...
				$mce_buttons_2 = array( 'formatselect', 'fontselect', 'underline', 'alignjustify', 'forecolor',  'backcolor', 'pastetext', 'removeformat', 'charmap', 'outdent', 'indent', 'undo', 'redo' );
	...
	public static function wp_mce_translation( $mce_locale = '', $json_only = false ) {
		...
		$baseurl = self::$baseurl ? self::$baseurl : plugins_url('', __FILE__) . '/tinymce';
	...
	public static function editor_js() {
		...
		if ( $tmce_on ) {
			...
			echo "<style>#chanai-candywin,.imeselector,.ime-settings{z-index:100101}</style>";
